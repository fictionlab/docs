...
[software structure](/leo-rover/1.8/documentation/specification#software-structure).
Among them, the rosserial server node (called `/serial_node` in this case)
"bridges" communication with the CORE2 board, so any topics it publishes or
subscribes are created and used in the firmware.
...
[Connect to the Internet tutorial](/leo-rover/1.8/guides/connect-to-network)).
