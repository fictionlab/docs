...
In this tutorial we will go through the process of connecting a RealSense D435i
...
topics. You can find all available topics and configuration options
[here](https://github.com/realsenseai/realsense-ros?tab=readme-ov-file#published-topics).
...
<div className="video-container">
  <LiteYouTubeEmbed
    ...
    poster="hqdefault"
    title="Intel depth camera on Leo Rover"
    webp
  />
</div>
...
We also want the description package, which includes 3D models of the device
...
with properties. We need to include the correct macro to make the camera visible
with the robot's model. To do that create a new file in `/etc/ros/urdf`
directory, and name it `realsense.urdf.xacro`. In this file, include the
following content:
...
by typing below commands in terminal:
...
We can create a `realsense.launch.xml` launch file in `/etc/ros` directory, that
would start the node with a fitting configuration.
...
To be able to see the camera model in RViz, you need to have the
`realsense2_description` package installed on your computer as well.
...
[Integrations page](/leo-integrations) for more hardware you can connect to your
rover.
