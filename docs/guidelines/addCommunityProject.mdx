...
it not meets the guidelines or is deemed inappropriate.
...
Fictionlab documentation uses custom React components to enhance the content
and provide additional functionality. To see a list of available components,
visit the
[Components](/guidelines/components) page.

## Creating a community project page

To add a community project to the Fictionlab documentation, create a new MDX
file in the `docs/community-projects` directory of this repository. Each
community project should have its own MDX file named after the project (e.g.,
...
be placed in the `/static/img/communityProjects` directory of this repository.
...
authors:
  - name: Author Name
    ...
    image_url: /path/to/author-image.jpg
    socials:
      x: author_x_handle
...
company:
  ...
  logo_url: /path/to/company-logo.jpg
---
```
...
Check existing community project pages in the `docs/community-projects`
directory for examples of how to structure the front matter and content.
...
request includes the new MDX file in the `docs/community-projects` directory.
