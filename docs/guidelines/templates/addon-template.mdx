...
1. Create new [ProductPreview](../components#productpreview) partial markdown
   and put it inside `src/products/addon_name.mdx` file.
1. Create new `addon-name.mdx` addon file inside the `addons` directory.
1. When copying and filling this template, remove all comments
   (`{/* comment example  */}` ).
1. When copying directly from `.mdx` file, remove all code blocks (only fencing,
   leave content).
1. Inline code indicates that this fragment needs to be replaced with other
...
import Product from '@site/src/products/addon_name.mdx'; // use product file that you created as prerequisite
...
`Product Preview - imported from src/products/addon_name.mdx`
