...
For specifications of the previous version of Leo Rover, check out the page:

<FlexTable width="100%">
  <FlexTableItem style={{ padding: '0 0.5rem 0 0.5rem' }}>
    <LinkButton docId="leo-rover/documentation/specification/specification-1.8" />
  </FlexTableItem>
  <FlexTableItem style={{ padding: '0 0.5rem 0 0.5rem' }}>
    <LinkButton docId="leo-rover/documentation/specification/specification-1.7" />
  </FlexTableItem>
</FlexTable>
...
| Estimated max. obstacle size |                ~70 m                |
...
| Climb grade (no payload)     | 45° ( ~58%) |
| Climb grade with 5kg payload | 45° ( ~36%) |
| Hill grade traversal         | 45° ( ~58%) |
| Nominal torque               |    4 Nm     |
...
<ThemedImageZoom
  ...
  sources={{
    light: useBaseUrl(
      '/img/robots/leo/specification/leo-1.9_dimensions-black.webp',
    ),
    dark: useBaseUrl(
      '/img/robots/leo/specification/leo-1.9_dimensions-white.webp',
    ),
  }}
  width="1300"
  height="1800"
...
| ----------------: | :------: | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Built-in computer |    1     | Raspberry Pi 5 - A single-board computer developed by Raspberry Pi Ltd.<br /> <br /> Key features: <br /> - Processor: Broadcom BCM2712, 2.4GHz quad-core 64-bit Arm Cortex-A76 CPU with cryptography extensions. 1 <br /> - Memory: 4GB LPDDR4X-4267 SDRAM. <br /> - Storage: Micro SD card slot with support for high-speed SDR104 mode; PCIe 2.0 x1 interface for fast peripherals (requires separate 3 M.2 HAT or adapter).<br /> - Connectivity: Gigabit Ethernet (supports PoE+ with separate PoE+ HAT), Dual-band 802.11ac Wi-Fi, Bluetooth 5.0 / Bluetooth Low Energy (BLE), 2 x USB 3.0 ports supporting simultaneous 5Gbps operation, 2 x USB 2.0 5 ports, 2 x micro-HDMI ports supporting resolutions up to 4kp60 with HDR support, 2 x 4-lane MIPI camera/display transceivers, standard 40-pin GPIO header. |
...
<ThemedImageZoom
  alt="Leo Rover 1.9 hardware diagram"
  sources={{
    light: useBaseUrl(
...
|                        Parameter |         Value         |
| -------------------------------: | :-------------------: |
| Tire size (diameter x thickness) |  ~ 125 mm x ~ 70 mm   |
...
The Rover is equipped with a 2A 12,6V battery charger that can charge the
battery in approximately 4 hours. The charger is designed to be compact and
portable, making it easy to transport and use in various environments. The
charger is equipped with a standard 5.5/2.1 mm DC plug. Together with the DC
5.5/2.1 mm - Weipu SP13-3 adapter (included with each rover), it can be used to
charge the Rover's battery from a standard power outlet.
...
1. Ensure the Rover is powered off.
2. Remove the battery from the Rover.
3. Unplug the Weipu SP13-3 connector from the battery.
4. Connect the battery to the charger using the provided Weipu SP13-3 - DC
   5.5/2.1 mm adapter.
5. Plug the charger into a standard power outlet.
6. Press the power button on the battery to initiate the charging process.

:::note

To charge the battery, power button on the must be pressed.
...
{/* TODO for now copied from Rapha Rover and merged with 1.8 */}
