...
- `heading_controller/cmd_vel` ([geometry_msgs/msg/Twist])
  ...
  steering and maintain its heading while driving straight or making wide
  turns. Only `linear.x` $[\frac{m}{s}]$ and `angular.z` $[\frac{rad}{s}]$
  are used. This topic is not supported when using mecanum wheels.
...
- `angular_hold_deadband` (type: `float`, default: `0.2`)
  ...
  Maximum absolute `angular.z` command still treated as heading-hold mode.
  Above this value, no heading correction is applied.
