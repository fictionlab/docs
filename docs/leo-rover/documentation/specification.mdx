...
Nodes][ros-nodes], treated as computational units, each performing one logical
function. The nodes interact via:
...
- **Controller firmware** - The firmware itself acts as a ROS node but uses
  eProsima's [Micro XRCE-DDS][micro-xrce-dds] as its middleware. Thus, it
...
- **Web User Interface** - The WebUI establishes a connection with the Rosbridge
  ...
  for communication with the ROS nodes.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Camera Driver</td>
      <td>
        Publishes camera info and various camera images including:

        - Color image.
        - Monochrome image.
        - Rectified images in color and monochrome.

        All images are published at 15 FPS in their original resolution
        and their compressed versions.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        Fuses wheel odometry and filtered IMU data to compute robot pose and velocity and publishes it.
        It optionally broadcasts an odometry TF transform and supports resetting the odometry via
        a service.
      </td>
    </tr>
    <tr>
      ...
      <td>
        Provides HTTP video streaming of ROS image topics for the Web User Interface.
      </td>
    </tr>
    <tr>
...
The "Firmware" refers to the low-level software running on the LeoCore
controller. It utilizes [Micro-ROS][micro-ros] to communicate with the built-in
...
communicates with the Rosbridge server using [roslibjs] to access
functionalities that are available on the ROS topics. The default [leo_ui]
brings features such as:

- control of the Rover via a keyboard or a virtual joystick
- display of a camera stream from the Web video server
...
- an HTTP server which hosts the Web User Interface,
- a desktop environment and a remote desktop access (in the "full" variant),
...
summarized by the following files:
...
- **/etc/ros/robot.launch.xml** - a launch file in XML format that starts the
  ...
  package which starts the base functionality of the rover. Can be extended by
  the user to, for example, add additional nodes to be started at boot.
...
- **/etc/ros/urdf/robot.urdf.xacro** - the URDF description (in [xacro] format)
  ...
  model from the [leo_description] package. Can be extended by the user to add
  additional links or joints to the model.
...
Apart from that, [systemd user units][systemd-user-units] are used to start the
ROS nodes at boot. The following units are defined:
...
Both of the services can be configured through the `/etc/ros/setup.bash` file.

To facilitate the process of managing these services, LeoOS comes with a set of
bash aliases:
