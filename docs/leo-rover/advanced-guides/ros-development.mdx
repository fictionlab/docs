...
[first section](../documentation/specification#software-specification) of this
tutorial. Among them, the rosserial server node (called `/serial_node` in this
case) "bridges" communication with the CORE2 board, so any topics it publishes
or subscribes are created and used in the firmware.
