...
this, you have to start ArUco tracking by setting the corresponding environment
...
Do not mix up the ways of starting the ArUco tracker: it should be either
enabled by setting `START_ARUCO_TRACKING=true` or by passing the argument to
`follow_aruco_marker.launch.xml`. If you use both ways of starting ArUco tracker
at once, the follower will not work.

:::

The rover will start following the marker once it detects the ArUco marker.
...
To make tracker parameter changes persist across restarts, edit
`/etc/ros/aruco_tracker.yaml` directly on the rover. This is available only if
you enabled the tracking by setting `START_ARUCO_TRACKING=true` and restarting
the ROS nodes.
...
#### Using a custom tracker parameters file

To reuse your tracker parameter settings across runs, create a `yaml`
