...
This instructions will fail if you don't have a ros workspace on the rover.
...
You need to source the python virtual environment in each terminal on the rover
you want to use for running the nodes. Othervise the nodes will fail because
they won't find the TensorFlow Lite library. <br/> You can source the
...
As you run this command, the robot won't move until you change the
...
provided description in a comment. Go through it before running, and check if
