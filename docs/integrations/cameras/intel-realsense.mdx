...
change `robot.urdf.xacro` file in `/etc/ros/urdf` directory. On
[this](https://github.com/IntelRealSense/realsense-ros/tree/ros2-master/realsense2_description/urdf)
site you can see what RealSense urdf camera models are available in the
...
have to put the same value as in the `urdf` file. We set here the
...
to global namespace makes it easier to distinguish the node's topics.
