...
You will also learn how to use the camera's on-device AI capabilities by
running an object detection example. The image below shows its final result -
the camera stream with the detected objects marked with bounding boxes,
visualized in RViz:

<ImageZoom
  src="/img/robots/leo/integrations/oak-1/oak-1-detection-example.webp"
...
Now you can switch to the PC connected to rover's access point and run the RViz:
...
bounding boxes and names of the detected objects:
