---
title: OAK-1 Leo Rover ROS 2 Setup
sidebar_label: Luxonis OAK-1
description: >-
  Integrate Luxonis OAK-1 with Leo Rover in ROS 2 Jazzy. Step-by-step mechanical mounting, USB-C wiring,
  URDF xacro setup, and on-device AI object detection.
image: /img/robots/leo/integrations/oak-1/OAK-1_integration_action_shot.webp
---
...
<ImageZoom
  loading="eager"
  src="/img/robots/leo/integrations/oak-1/OAK-1_integration_leo_action_shot.webp"
...
in the package. It is designed to mount the OAK-1 camera on top of the rover.
...
3. Mount the subassembly in the designated place on the Leo Rover - the correct
   positioning is shown in the image below.
4. Plug one end of the USB-C cable into the angled USB adapter and the other end
   into the Rover's external USB port.
...
[addon adapters](../addon-adapters#oak-1-adapter) page.
...
permissions. Log in to the rover via SSH, and create the `luxonis.rules` file in
...
by running _RQt_ on your computer with ROS installed and connected to the rover
network:
...
out the
[depthai-ros documentation](https://docs.luxonis.com/software/ros/depthai-ros/driver).
...
Now your OAK-1 camera ROS node will start at launch. You can also start it now
by typing
