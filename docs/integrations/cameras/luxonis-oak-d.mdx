...
in the package. It is designed to mount the OAK-D camera on top of
the rover.
...
To mount the sensor:
1. Insert the right-angle adapter into the USB-C port so that it faces backwards. Then, connect the OAK Y-adapter to the angle adapter.
2. Insert the OAK-D camera into the adapter and secure it with the M4 x 8 bolts. Ensure that the camera's USB port faces downwards.
3. Place the assembly on top of the rover and align the mounting holes with those in the front row of the rover. The camera should be perfectly centered.
...
The OAK-D camera requires a relatively high supply current. Therefore, we strongly recommend using the [Powerbox](/leo-rover/addons/powerbox) to provide an additional 5V power supply.
<Powerbox />
