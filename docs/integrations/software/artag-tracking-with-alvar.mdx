---
title: ArUco-Tag tracking with OpenCV
sidebar_label: ArUcoTag tracking with OpenCV module
keywords:
  - aruco
  - tracking
  - opencv
...
The detected AR Tags are also published to `/aruco_detections` topic, so you
...
To launch the tracker you can, create your own launch file, use the one from the
`aruco-opencv` package or you can use the provided launch file which provides
basic config for detecting single markres.
...
You can check
[here](https://docs.opencv.org/4.2.0/d5/dae/tutorial_aruco_detection.html)
description of each parameter in the `aruco` namespace.

:::

Next, also in `/etc/ros` create a launch file that will use the previously added
configuration with the tracker node.
...
```bash
ros2 run /etc/ros/tracker.launch.xml
```
...
<include file="/etc/ros/tracker.laumch.xml"/>
```
