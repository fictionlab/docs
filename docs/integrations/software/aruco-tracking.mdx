...
is working - either with `ros2 param set` or using the `rqt_reconfigure` GUI tool.
Feel free to play around with different configurations and then add the chosen parameters
to the `/etc/ros/aruco_tracker.yaml` file.
