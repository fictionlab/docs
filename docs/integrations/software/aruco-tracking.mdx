...
File `/etc/ros/aruco_tracker.yaml` describes configuration of ArUco Marker
Tracking. Among others, by default it is configured to tract markers from
`4X4_50` ArUco dictionary with marker side size of 15cm. If you already have
different ArUco markers, `/etc/ros/aruco_tracker.yaml` has to be adjusted
appropriately.
