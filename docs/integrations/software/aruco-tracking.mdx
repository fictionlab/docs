...
Start aruco tracking by setting the corresponding environment variable to `true` in `etc/ros/setup.bash`:
```bash
# Set to true to enable aruco tracking
export START_ARUCO_TRACKING=true
```

Source the modified setup file:
...
```bash
ros2 launch /etc/ros/robot.launch.xml
```

:::info

File `/etc/ros/aruco_tracker.yaml` describes configuration of ArUco Marker Tracking. Among others, by default it is configured to tract markers from `4X4_50` ArUco dictionary with marker side size of 15cm. If you already have different ArUco markers, `/etc/ros/aruco_tracker.yaml` has to be adjusted appropriately.

The parameters in the `aruco` namespace can be changed at runtime while the
node is working - either with `ros2 param set` or using the `rqt_reconfigure`
GUI. You can play with their values and then add the chosen parameters to the
