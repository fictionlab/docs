...
Assembly require using two Allen keys that are not included in the package - sizes 2 mm and 4 mm.
...
2. Connect the JST connector to the TTL-to-USB adapter and insert the adapter into the plastic mount. Tuck the excess cable into the cavity. The completed assembly should resemble the image below.
...
If other USB devices are connected to Leo Rover it may be required to connect 5V power to TTL-to-USB adapter. 

In this case we recommend using powerbox addon and modified barrel jack cable. Contact us to get details about modifying the cable.
