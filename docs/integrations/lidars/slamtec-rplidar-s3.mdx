...
the lidar's field of view if it sticks up too much. Try to hide it in the empty
space inside Leo Rover's back frame. Just like on the photo below:
...
means that no external power sources are necessary. However, if you want a
cleaner and more reliable power and data connection, we recommend using the
Powerbox addon:
