...
in the package. It is designed to mount the RPLidar A2 on top of
the rover.
...
Some USB-UART adapters allow you to choose the baud rate. If your adapter has this option, make sure that the switch is set to 115200 bps.
An incorrectly set baud rate may cause problems with receiving LiDAR data.

:::

2. Secure the RPLidar A2 to the 3D-printed mount using M3x6 Allen head screws. Make sure that the sensor is rotated correctly - the lidar cable should point towards the back of the rover. Image of correct mounting is presented in wiring section.
...
Make sure that excess cable is wrapper around the sensor so it does not obstruct LiDARs field of view.
...
means that no external power sources are necessary. With everything connected, you are ready to try out your new sensor.
